...
func ExampleUsage() {
	...
	set, err = GetInstance().SetNX(ctx, "key", "value", redis.KeepTTL).Result()
	log.Println("SetNX", set, "err", err)

	// SORT list LIMIT 0 2 ASC
	val3, err := GetInstance().Sort(ctx, "list", &redis.Sort{Offset: 0, Count: 2, Order: "ASC"}).Result()
	log.Println("Sort", val3, "err", err)

	// ZRANGEBYSCORE zset -inf +inf WITHSCORES LIMIT 0 2
	val4, err := GetInstance().ZRangeByScoreWithScores(ctx, "zset", &redis.ZRangeBy{
		Min:    "-inf",
		Max:    "+inf",
		Offset: 0,
		Count:  2,
	...
	val5, err := GetInstance().ZInterStore(ctx, "out", &redis.ZStore{
